...
- 当前 release: [v0.2.0](https://github.com/JarvisTech-001/oss-triage-brief/releases/tag/v0.2.0)。
- CI 会在 push 和 pull request 上运行 `npm run check`。
- npm 发布状态用 [issue #2](https://github.com/JarvisTech-001/oss-triage-brief/issues/2) 跟踪。
- `v0.2.0` 增加了 GitHub JSON 输入、PR diff 上下文、release brief、示例和
  GitHub Action 集成示例。
...
- 通过 `--from-json` 直接读取 `gh` 输出的 GitHub issue 和 PR JSON。
- 通过 `--diff` 或 `--diff-file` 汇总 PR diff 大小和变更文件。
- 生成 release checklist brief，辅助维护者判断是否可以发布。
